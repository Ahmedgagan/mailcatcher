...
module MailCatcher::Mail
  ...
  def db
    @__db ||= SQLite3::Database.new(':memory:', type_translation: true).tap do |db|
      db.execute(<<-SQL)
  ...
  def http_url
    "http://#{MailCatcher.options[:http_ip]}:#{MailCatcher.options[:http_port]}#{MailCatcher.options[:http_path]}"
  end
